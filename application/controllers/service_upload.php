...
class Service_upload extends CI_Controller {
	...
	function upload_service() {
		...
		$data['main_content'] = 'upload_service';
		$data['data']['title'] = '上传其他二手物品或Service';
		$this->load->view('includes/template', $data);
	}
	...
	function upload_validation() {
		...
		$this->form_validation->set_rules('remain', '数量', 'trim|required|is_numeric');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($query = $this->service_model->add_service()) {
				redirect('service_details/service/'.$query);
			}
			else {
				$this->index();
			}
