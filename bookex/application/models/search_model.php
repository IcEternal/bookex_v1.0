<?php 
	class Search_model extends CI_Model{
		...
		function getResult(){
			...
			if (strlen($key) == 0)
				$query = "SELECT * FROM book WHERE $condition ORDER BY id DESC LIMIT $begin, 20;";
		...
		function getUserspaceResult($err = 0){
			...
			jd_stopattack();
			$user = $this->session->userdata('username');
			$query = "SELECT * FROM book WHERE (subscriber = \"$user\" AND finishtime = null AND finishtime = \"0000-00-00 00:00:00\");";
			$result1 = $this->db->query($query)->result();
			$query = "SELECT * FROM book WHERE (uploader = \"$user\" AND subscriber != \"N\" AND finishtime = \"0000-00-00 00:00:00\");";
			$result2 = $this->db->query($query)->result();
			$query = "SELECT * FROM book WHERE (uploader = \"$user\" AND finishtime = \"0000-00-00 00:00:00\") ;";
			$result3 = $this->db->query($query)->result();
			$query = "SELECT * FROM book WHERE ((uploader = \"$user\" OR subscriber = \"$user\") AND finishtime != \"0000-00-00 00:00:00\") ;";
